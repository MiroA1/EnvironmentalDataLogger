...
public class AirQualityDataExtractor implements DataExtractor {
    ...
    public TreeMap<Date, Double> parseAirQualityData(String json, String param) {
        ...
        try {
            ...
            for ( int i = 0; i < dateArray.length(); i++) {
                ...
                Date date = parseDate(dateObject);
                if (date != null) {
    ...
    private Date parseDate(String dateString) {
        try {
            return new SimpleDateFormat("yyyy-MM-dd'T'HH:mm").parse(dateString);
